...
from .longport_source import DEFAULT_ADJUST, candles_to_frame, fetch_raw_candles
...
class DataFeed:
    ...
    def __init__(self, indicator_calculator: IndicatorCalculator | None = None):
    ...
    def _apply_indicators(self, frame: pd.DataFrame) -> pd.DataFrame:
        """顺序计算 EMA/MACD/RSI/ATR，确保同一 DataFrame 持续扩展。"""

        enriched = self.indicator_calculator.compute_ema(frame)
