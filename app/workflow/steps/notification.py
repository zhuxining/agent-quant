...
async def _notification(step_input: StepInput) -> StepOutput:
    ...
    if risk_output and risk_output.content:
        content = risk_output.content
        if isinstance(content, dict):
            approved_actions_count = len(content.get("approved_actions", []))

    executed_count = 0
    failed_count = 0
    errors: list[str] = []
    trade_output = previous_outputs.get("Execute Trades")
    if trade_output and trade_output.content:
        content = trade_output.content
        if isinstance(content, dict):
            executed_count = content.get("executed_count", 0)
            failed_count = content.get("failed_count", 0)
            summary = content.get("execution_summary")
